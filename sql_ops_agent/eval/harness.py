...
class MockLLM:
    ...
    async def chat(self, messages: List[ChatMessage], *, temperature: float = 0.0, stop=None) -> ChatResult:
        ...
        if "top 5 orders" in query:
            ...
            resp_obj = {"plan": "Insufficent info", "sql": None, "answer_text": "I don't know.", "citations": []}
            ...
            resp_obj = {"plan": "Unknown", "sql": None, "answer_text": "I don't know.", "citations": []}

        return ChatResult(
            text=json.dumps(resp_obj), usage={"prompt_tokens": 100, "completion_tokens": 50, "latency_ms": 50}
        )
...
class EvalHarness:
    ...
    async def run(self):
        ...
        for case in self.cases:
            ...
            self.results.append(CaseResult(case_id=cid, passed=passed, details={"log": log, "outcome": result.outcome}))
